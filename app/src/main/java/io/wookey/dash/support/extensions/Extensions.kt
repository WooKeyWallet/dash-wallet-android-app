package io.wookey.dash.support.extensions
...
fun String.formatterAmount(digit: Int = 8, defaultStr: String = "0.00"): String {
    if (isNullOrBlank()) {
        return defaultStr
    }
    try {
...
fun String.formatterAmountStrip(defaultStr: String = "0.00"): String {
    if (isNullOrBlank()) {
        return defaultStr
    }
    try {
...
fun View.showTimePicker(
        startDate: Calendar = Calendar.getInstance().apply { set(2014, 0, 19) },
        listener: (Date) -> Unit
) {
    setOnClickListener {
        hideKeyboard()
...
fun Pair<String, String>.formatMoney() = second.formatterAmount(2, "--").formatMoney(first)

fun String.formatMoney(currency: String = ExchangeRatesHelper.instance.currency): String {
    if (isBlank()) return "--"
...
fun String.formatRate(defaultStr: String = "--"): String {
    if (isBlank()) return defaultStr
    return try {
        val value = BigDecimal(this)
        val r = ExchangeRatesHelper.instance.getRate()
        if (r.isBlank()) {
            defaultStr
        } else {
            val rate = BigDecimal(r)
            value.multiply(rate).toPlainString().formatterAmount(2, defaultStr).formatMoney()
        }
    } catch (e: Exception) {
        e.printStackTrace()
        defaultStr
...
fun String.formatRateWithOutSymbol(defaultStr: String = "0.00"): String {
    if (isBlank()) return defaultStr
    return try {
        val value = BigDecimal(this)
        val r = ExchangeRatesHelper.instance.getRate()
        if (r.isBlank()) {
            ...
            val rate = BigDecimal(r)
            value.multiply(rate).toPlainString().formatterAmount(2, defaultStr)
        }
    } catch (e: Exception) {
        e.printStackTrace()
        defaultStr
...
fun String.formatRateReverse(defaultStr: String = "0.00"): String {
    if (isBlank()) return defaultStr
    return try {
        val value = BigDecimal(this)
        val r = ExchangeRatesHelper.instance.getRate()
        if (r.isBlank()) {
            ...
            val rate = BigDecimal(r)
            value.divide(rate, 8, BigDecimal.ROUND_HALF_UP).toPlainString().formatterAmount(8, defaultStr)
        }
    } catch (e: Exception) {
        e.printStackTrace()
        defaultStr
...
fun String.parseURI(): Pair<String, String>? {
    val indexOfScheme = indexOf(":")
    if (indexOfScheme != -1) {
        val scheme = substring(0, indexOfScheme)
        if (!scheme.equals("DASH", true)) {
            return null
        }
        val indexOfParams = indexOf("?")
        if (indexOfParams == -1) {
            return Pair(substring(indexOfScheme + 1), "")
        }
        val params = substring(indexOfParams + 1)
        val amounts = params.split("&").filter {
            it.startsWith("amount=")
        }
        return if (amounts.size == 1) {
            val amount = amounts[0]
            Pair(substring(indexOfScheme + 1, indexOfParams), amount.substring(amount.indexOf("=") + 1))
        } else {
            Pair(substring(indexOfScheme + 1, indexOfParams), "")
        }
    }
    return null
}