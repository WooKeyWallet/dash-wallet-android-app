...
import io.wookey.dash.feature.generate.GenerateWalletActivity
...
class WalletManagerActivity : BaseTitleSecondActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        AppDatabase.getInstance().walletDao().loadSymbolWallets("DASH").observe(this, Observer { value ->
            value?.let {
                list.clear()
                list.addAll(it)
        ...
        viewModel.switchWallet.observe(this, Observer {
            finish()
        })
    ...
    class WalletAdapter(val data: List<Wallet>, private val viewModel: WalletManagerViewModel) :
        ...
        class ViewHolder(override val containerView: View, val viewModel: WalletManagerViewModel) : RecyclerView.ViewHolder(containerView), LayoutContainer {
            fun bindViewHolder(wallet: Wallet) {
                ...
                rate.text = wallet.balance.formatRate("0.00")
